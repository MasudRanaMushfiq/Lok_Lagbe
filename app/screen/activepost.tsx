...
export default function AllPostsScreen() {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      ...
      setPosts(postsList);
      setFilteredPosts(postsList.filter((p) => p.status === 'active'));
    } catch (error) {
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <LinearGradient
        ...
        style={[
          ...
          {
            paddingTop:
              (Platform.OS === 'android' ? StatusBar.currentHeight || 24 : 20) + 8,
          },
        ]}
      >
          ...
          filteredPosts.map((post) => (
            ...
            >
              <View style={styles.workCard}>
                <TouchableOpacity
                  onPress={() =>
                    router.push({ pathname: '/works/[work]', params: { work: post.id } })
                  }
                >
...
const styles = StyleSheet.create({
  ...
  workCategory: { fontSize: 13, color: '#1877F2', marginBottom: 8, fontStyle: 'italic' },
