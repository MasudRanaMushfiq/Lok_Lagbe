    ...
    const fetchAcceptedWorks = async () => {
      ...
      try {
        ...
        const works: WorkData[] = (
          ...
          .filter(Boolean)
          .filter((work) => work?.status === 'accepted_sent') as WorkData[];
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#E6F2FF' }}>
      ...
      >
        ...
        <Text style={styles.headerTitle}>Work Request Sent</Text>
      </LinearGradient>

      <ScrollView contentContainerStyle={{ ...styles.container, paddingTop: 20 }}>
        {acceptedWorks.length === 0 ? (
          <Text style={styles.noWorksText}>No works are applied</Text>
        ) : (
...
const styles = StyleSheet.create({
  ...
  acceptedStatus: { color: '#3a125d', fontWeight: '700' },
